...
class KnxConnection():
    ...
    def find_knx_gateways(self):
        ...
        multicast_packet = scapy.all.IP(dst='224.0.23.12')/scapy.all.UDP()/scapy.all.Raw(load=knx_search_request.get_message())
    ...
    def _establish_knx_tunnel(self, knx_gateway):
        ...
        packet = scapy.all.IP(dst=knx_gateway)/scapy.all.UDP()/scapy.all.Raw(load=knx_connection_request.get_message())
